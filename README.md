...
Using as input a BED format file CRISPETA is able to find, analyze, and score all posible gRNAs. As a result the program returns:

1. A file with {n} paired gRNAs per region.
2. A log file with summary of the execution.
3. A BED file with genome coordinates of gRNAs ready to be uploaded to Genome Browser.
4. PDF with graphics based on summary numbers
5. HTML with same graphics than the PDF above to open in browser and interact with them

The code for the tool can be found on github: https://github.com/...
...
- **-o dir (default: ./gRNA_pairs):** Path where outputs will be placed
...
- **-v float (default: 0.5):** Percentage of maximum variety in gRNA pairs 
- **-eu int (default: 100):** Distance upstream from target region for the program to start to look for gRNAs
- **-ed int (default: 100):** Distance downstream from target region for the program to start to look for gRNAs
- **-du int (default: 500):** Range upstream from the excluded region to look for gRNAs (value must be positive)
- **-dd int (default: 500):** Range downstream from the excluded region to look for gRNAs (value must be positive)
- **-mp dir:** Path to BED file with positive regions. gRNAs overlapping positive regions will be favored when ranking result pairs
- **-mn dir:** Path to BED file with negative regions. gRNAs overlapping negative regions will be disfavored when ranking result pairs
- **-si float (default: 0.2):** Minimum individual gRNA score allowed
- **-sp float (default: 0.4):** Minimum gRNA paired score allowed
- **-sc string (default: +):** Method applied to paired score: "+" (sum) or "x" (product) of individual gRNA scores. Multiplication method will favor those gRNA pairs where both gRNAs have high scores e.g: sum 0.8+0.2=1, product 0.8*0.2=0.16.
- **-c string (default: None):** Method applied when making gRNA pairs: "None" or "DECKO". DECKO method will create pairs where at least one of the gRNAs starts with G.
...
1. Main output: Pairs found by CRISPETA for target regions in Input file
		...
		Sequence_ID(#pair)	chromosome	start	end	gRNA_1+PAM	score_1	chromosome	start	end	gRNA_2+PAM	score_2	distance_to_exclude_up_region	distance_to_exclude_down_region	distance_between_gRNAs	paired_score	mask_score	oligo
...
2. GenomeBrowser file: Pairs and target regions in BED format ready to upload in GenomeBrowser as a custom traks
		...
		track name="Target_Regions" description="Regions_for_gRNA_searching" visibility=1 itemRgb="On"
		...
		...
		track name="gRNA_pairs" description="gRNA_pairs" visibility=3 itemRgb="On"
